...
/*
 * ZFS encryption implementation for OSv - adapted from the Linux/ICP version.
 *
 * OSv uses FreeBSD-style zfs_uio_t (wrapping struct uio * via GET_UIO_STRUCT).
 * This file is a modified copy of module/os/linux/zfs/zio_crypt.c with these
 * OSv-specific adaptations:
 *   1. zfs_uio_t field access uses GET_UIO_STRUCT(), zfs_uio_iovcnt(),
 *      instead of direct struct member access.
 *   2. Stack-allocated zfs_uio_t objects are backed by a struct uio and
 *      initialized with zfs_uio_init().
 *   3. uio_segflg assignments are no-ops: OSv always uses ZFS_UIO_SYSSPACE.
 *   4. module_param / MODULE_PARM_DESC are suppressed (no kernel module params).
 *
 * Original copyright (c) 2017, Datto, Inc. All rights reserved.
 */
    ...
    zfs_uio_t *puio, zfs_uio_t *cuio, uint8_t *authbuf, uint_t auth_len)
{
	...
	maclen = GET_UIO_STRUCT(cuio)->uio_iov[zfs_uio_iovcnt(cuio) - 1].iov_len;
    ...
    uint8_t *mac, uint8_t *keydata_out, uint8_t *hmac_keydata_out)
{
	...
	(void)memset(&puio_s, 0, sizeof (puio_s));
	(void)memset(&cuio_s, 0, sizeof (cuio_s));
	...
	GET_UIO_STRUCT(&puio)->uio_iov = plain_iovecs;
	zfs_uio_iovcnt(&puio) = 2;
	/* OSv: uio_segflg is always ZFS_UIO_SYSSPACE, assignment skipped */
	GET_UIO_STRUCT(&cuio)->uio_iov = cipher_iovecs;
	zfs_uio_iovcnt(&cuio) = 3;
	/* OSv: uio_segflg is always ZFS_UIO_SYSSPACE, assignment skipped */
...
zio_crypt_destroy_uio(zfs_uio_t *uio)
{
	if (GET_UIO_STRUCT(uio)->uio_iov)
		kmem_free(GET_UIO_STRUCT(uio)->uio_iov, zfs_uio_iovcnt(uio) * sizeof (iovec_t));
}
    ...
    boolean_t *no_crypt)
{
	...
	*auth_len = aad_len;
	...
	if (encrypt) {
		GET_UIO_STRUCT(puio)->uio_iov = src_iovecs;
		zfs_uio_iovcnt(puio) = nr_src;
		GET_UIO_STRUCT(cuio)->uio_iov = dst_iovecs;
		zfs_uio_iovcnt(cuio) = nr_dst;
	} else {
		GET_UIO_STRUCT(puio)->uio_iov = dst_iovecs;
		zfs_uio_iovcnt(puio) = nr_dst;
		GET_UIO_STRUCT(cuio)->uio_iov = src_iovecs;
		zfs_uio_iovcnt(cuio) = nr_src;
	}
...
error:
	...
	*auth_len = 0;
	*no_crypt = B_FALSE;
	GET_UIO_STRUCT(puio)->uio_iov = NULL;
	zfs_uio_iovcnt(puio) = 0;
	GET_UIO_STRUCT(cuio)->uio_iov = NULL;
	zfs_uio_iovcnt(cuio) = 0;
	return (ret);
}
    ...
    uint_t *enc_len)
{
	...
	*enc_len = datalen;
	GET_UIO_STRUCT(puio)->uio_iov = plain_iovecs;
	zfs_uio_iovcnt(puio) = nr_plain;
	GET_UIO_STRUCT(cuio)->uio_iov = cipher_iovecs;
	zfs_uio_iovcnt(cuio) = nr_cipher;
    ...
    uint8_t **authbuf, uint_t *auth_len, boolean_t *no_crypt)
{
	...
	mac_iov = ((iovec_t *)&GET_UIO_STRUCT(cuio)->uio_iov[zfs_uio_iovcnt(cuio) - 1]);
    ...
    boolean_t *no_crypt)
{
	...
	/*
	 * Do NOT memset puio/cuio here. zfs_uio_init() already initialises
	 * the zfs_uio_t and sets uio->uio to point at the backing struct uio.
	 * A second memset would clear that pointer to NULL, causing a NULL
	 * dereference inside zio_crypt_init_uios_*() at GET_UIO_STRUCT().
	 */
...
#if defined(_KERNEL)
/* OSv: module_param and MODULE_PARM_DESC suppressed */
#endif
