...
## Known failing tests under conf_zfs=openzfs

These existing OSv filesystem tests do not pass when run against an OpenZFS root
and are known gaps, not regressions from this work.  They are tracked and left
for follow-up; the ZFS feature itself (boot, pool import, root mount, r/w, and
the Tier 0-3 matrices above) is unaffected.

| Test | Why it fails under OpenZFS |
|---|---|
| tst-fallocate | posix_fallocate/preallocation semantics not implemented on the OpenZFS vnops path (same class as the ramfs statfs/free-blocks gap noted for the rofs image). |
...
| tst-zfs-mount | in-guest mount/remount sequence the test drives is not fully supported by the OSv OpenZFS mount shim. |

The long-running ZFS stress test tst-zfs-db-sim was renamed to misc-zfs-db-sim
so it is excluded from the default test run (it is not a failure, just slow).
