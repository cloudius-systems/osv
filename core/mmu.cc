...
static mremap_src inspect_mremap_src(uintptr_t start, size_t size, size_t grow)
{
    ...
    if (fvma) {
        s.file = fvma->file();
        // offset() is the file offset of the vma's *start*; when start lies in
        // the middle of the vma the source page's offset is shifted by the
        // in-vma distance, otherwise a sub-range remap would re-derive the
        // wrong file bytes.
        s.offset = fvma->offset() + (start - i->start());
    }
    uintptr_t end = start + size;
...
//
// TODO: anon and file-backed mappings are handled; the move path preserves
...
void* mremap(void* old_addr, size_t old_size, size_t new_size, unsigned flags)
{
    ...
    // old_size == 0 is only meaningful on Linux for creating a fresh mirror of
    // a shareable mapping; we do not implement that, and a zero old_size would
    // otherwise "grow" from nothing or munmap a zero-length tail.  Reject it.
    if (old_size == 0) {
        errno = EINVAL;
        return MAP_FAILED;
    }
    ...
    if (new_size <= old_size) {
        if (new_size < old_size) {
            // The source range was verified wholly mapped above, so this cannot
            // fail with EINVAL; treat any error defensively as ENOMEM rather
            // than reporting a success that left the mapping unchanged.
            auto e = munmap(reinterpret_cast<void*>(old_start + new_size),
                            old_size - new_size);
            if (e.bad()) {
                e.to_libc();
                return MAP_FAILED;
    ...
    if (src.is_file) {
        // A writable MAP_PRIVATE file mapping may hold COW-dirtied pages whose
        // modifications live only in this mapping's private copies.  Re-deriving
        // them from the file during the move would silently lose those writes,
        // and a naive populate+memcpy across the page-cache COW machinery
        // faults.  Until a correct private-dirty-page copy exists, refuse the
        // move rather than corrupt data.
        // TODO: copy private dirty pages across and drop this guard.
        if ((src.flags & mmap_shared) == 0 && (src.perm & perm_write)) {
            errno = EINVAL;
            return MAP_FAILED;
        }
