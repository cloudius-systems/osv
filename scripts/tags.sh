...
{
	include=$(find ${tree}arch/$ARCH/$1 $subarchprune \
...
{
	find ${tree}include/$1 $ignore -name config -prune -o -name "$2" \
		-print -o -name '*.hh' -not -type l -print;
}

# find sources in include/api
find_include_api_sources()
{
	find ${tree}include/api/$1 -name "$2" -print -not -type l -print;
}
...
all_sources()
{
	for dir in glibc-compat lockfree osv
	do	
		find_include_sources $dir '*.[chS]'
	done
	find_include_api_sources $SRCARCH '*.[chS]'
