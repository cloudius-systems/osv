...
spa_taskq_dispatch_ent(spa_t *spa, zio_type_t t, zio_taskq_type_t q,
    task_func_t *func, void *arg, uint_t flags, void *task)
{
	spa_taskqs_t *tqs = &spa->spa_zio_taskq[t][q];
	taskq_t *tq;
...
#ifdef _KERNEL
	(void) taskq_dispatch_safe(tq, func, arg, flags, (struct ostask *)task);
#else
	(void) taskq_dispatch(tq, func, arg, flags);
#endif
}
...
static int
zpool_get_config(const char *bootdev, nvlist_t ***configs_ptr,
    uint64_t *count)
{
	nvlist_t **configs;
	nvlist_t *config;
	...
	*configs_ptr = NULL;
	...
	/* Get boot vdev config */
	if (vdev_disk_read_rootlabel(bootdev, &config) != 0)
		return (-1);
	configs = (nvlist_t **) kmem_zalloc(sizeof(nvlist_t *), KM_SLEEP);
	if (!configs)
		return (-1);
...
spa_generate_rootconf(const char *name)
{
	...
	if (zpool_get_config(name, &configs, &count))
		return (NULL);
