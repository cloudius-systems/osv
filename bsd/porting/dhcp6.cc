                                         ...
                                         bool include_iaaddr)
{
    ...
    // (standard isc-dhcp/kea/dnsmasq as well as cloud DHCPv6 servers).
    if (type == DH6_SOLICIT)
...
void dhcp6_interface_state::bind_address()
{
    ...
    // supplied the on-link prefix, so use a /64 mask for typical setups.
...
void dhcp6_interface_state::process_packet(struct mbuf* m)
{
    ...
    if (_state == DH6_SOLICITING) {
        if (t == DH6_REPLY && _have_addr) {
            ...
            // with a REPLY directly, skipping ADVERTISE/REQUEST. Some cloud
            // DHCPv6 servers do this. Bind straight away.
            bind_address();
...
void dhcp6_worker::start(bool wait)
{
    ...
    // Real servers (and cloud DHCPv6 servers) can take a few seconds to a
